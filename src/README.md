...
1. 객체지향 프로그래밍이란 무엇인가?
   ...
   - 객체지향 프로그래밍은 일종의 프로그래밍 패러다임으로 객체(objects)의 개념에 기반하고 있다.
   - 객체는 데이터를 포함하여 필드의 형태를 띠고 있으며 이를 속성(attributes)이라고 부르기도 한다.
   - 객체는 코드를 포함하며 이것은 프로시저의 형식으로 되어있다. 종종 메서드(methods)라고 부른다.

   - 개념 잡기

     1. ```객체```는 ```데이터```와 ```코드```를 포함한다.

        2.객체의 데이터는 ```변수```다.

     3. 객체의 데이터는 실제로 상수, 변수, 객체를 포함한다.

     4. 객체의 코드는 객체의 데이터를 사용한다.

     5. 좋은 객체는 ```적절한``` 데이터를 포함하며 그 데이터를 활용하는 코드를 포함한다.
     6. 자바 언어는 객체를 만들 때 class 키워드를 사용한다.
     7. 클래스의 데이터는 멤버 변수라고 부른다.
     8. 클래스의 코드는 메서드(method)라고 부른다.
     9. 객체는 메서드(method = 사용법)를 호출하여 사용한다.
     10. 객체를 생성할 때는 new 키워드를 사용한다.
     11. this 키워드는 객체 자신을 의미합니다.
     12. 멤버 변수의 접근 제어자는 private을 원칙으로 합니다.(캡슐화)

     13. 생성자의 접근 제어자는 기본적으로 public을 사용하며 그 외에는 특별한 이유를 갖습니다.

     14. 메서드의 접근 제어자는 public, private, protected, (default)가 있습니다.
     15. static 키워드는 final 키워드와 함께 상수를 정의합니다.
         - 멤버 변수의 초기화
           - firstName과 lastName 멤버 변수는 초기값이 없음(생성자에서 결정되어야 함)
           - age, height, destination 멤버 변수는 초기값이 상수임(생성자에서 변경될 수 있음)

     16. static 메서드가 모여 종종 유틸리티 메서드가 됩니다.
         - 일반 메서드 vs static 메서드
           - 일반 메서드는 멤버 변수를 활용
           - static 메서드는 상수와 메서드 인수를 활용, 멤버 변수 활용할 수 없음

     17. final 키워드는 상수와 메서드의 인자에 충분히 붙여주세요.
         - final은 변경되지 않는 변수(값)
           - 한번 값을 할당하면 다른 값을 할당할 수 없다.

     18. 자바 클래스는 메서드의 이름은 같지만 인자의 개수와 종류가 다른 다수의 메서드를 가질 수 있습니다.

     19. 메서드 오버로딩을 사용할 때는 본질적으로 같은 메서드를 묶어야 한다.

         - 메서드 오버로딩
           - ex) public void accelerate(int km)

         - 메서드 오버로딩 하지 않음
           - ex) public void decelerateAs(int km)

     20. 객체 지향 프로그램의 묘미는 중복된 코드를 제거하는 것입니다.
     21. null 상태를 최소화해야 합니다. 객체 변수는 선언 후 최대한 빠르게 실제 값(객체)를 할당합니다.
         - null의 의미
           1. 객체 변수의 값이 없음
           2. 객체 변수에 값이 정해지지 않았음
           3. 객체 변수에 null 값을 지정 가능
           4. 객체 배열의 초기값도 null 임(일반 객체와 동일)

| 종류                | 예제 코드                          | 비고                                  |
| ------------------- | ---------------------------------- | ------------------------------------- |
| 상수(기본 데이터형) | int STORAGE_MAX = 100              | String은 기본 데이터형으로 포함하였음 |
| 변수(기본 데이터형) | String studentName                 |                                       |
| 객체                | 표준 API, 사용자가 정의한 클래스들 | API : 응용 프로그래밍 인터페이스      |

| 접근 제어자 | 의미                                                         | 기능                                                         | 비고                                      |
| ----------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ----------------------------------------- |
| public      | 공개함, 어디에서나 접근할 수 있음                            | 프로그램 어디서나 그 객체를 생성할 수 있음                   |                                           |
| private     | 비공개함, 같은 클래스에서만 접근할 수 있음                   | 해당 클래스 내에서만 그 객체를 생성할 수 있음, 기본 과정에서는 추천하지 않음 |                                           |
| protected   | 비공개함, 기본적으로 private와 동일하지만 상속한 하위 클래스에서는 공개함 | 부모 클래스의 생성자를 자식 클래스에서 그대로 사용해야 할 때 |                                           |
| (default)   | 같은 패키지에만 공개함, 생략하면 기본으로 적용됨             | 같은 패키지에서만 해당 클래스의 객체를 생성할 때             | 종종 원서에서 package-pricate 으로 표현됨 |
...
2. 객체 배열
   ...
   - 객체 배열 접근
     ...
     - int, double, String 배열과 사용법이 같음
     - ex) cars[0], cars[1] 등

3. null 의 개념

   - null 대응하기
     1. null이 오면 로직을 skip 합니다.(메서드의 나머지 내용을 실행하지 않음)
     2. null이 오면 다른 이름으로 교체합니다. ex) name = "기본 이름"
     3. null이 오면 default 인수로 대체합니다. ex) name = defaultName
